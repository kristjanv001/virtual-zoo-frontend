...
export class HologramCreatorComponent {
  ...
  hologramForm = new FormGroup({
    ...
    weight: new FormControl(0, [Validators.required, Validators.min(0), Validators.max(200000000)]),
    superPower: new FormControl("", [Validators.required, Validators.minLength(2), Validators.maxLength(1000)]),
  ...
  addHologram() {
    if (this.hologramForm.invalid) {
      this.hologramForm.markAllAsTouched();
      this.hologramForm.markAsDirty();
    } else {
